...
commons = {
	...
	generations = 1,						--[Optional] The number of generations to immediately compute after generation. Defaults to [0]. *
	...
	gridIterator = premade.iLeftRightDown,	--[Semi-Optional] A function defining the order in which to iterate over the grid.
}
...
---Ruleset data/params.
rules.initialLivingPercentage = 0.3
rules.survive = {min = 3, max = 8}
rules.born = {min = 3, max = 8}
...
rules.states.dead = {
	update = function(self, rules, adj, countedAdj, generation)
		local deadCount = countedAdj.dead
		local min = rules.born.min
		local max = rules.born.max
